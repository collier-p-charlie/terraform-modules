...
We need to `git add` first for new untracked files otherwise `pre-commit` will not detect them. Already-tracked files (e.g. `CHANGELOG.md`, `pyproject.toml`) do not need staging before `pre-commit` as `--all-files` covers them regardless.
